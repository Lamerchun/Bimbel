...
Voice (SLICE-3-VOICE-DRAG): hold empty-composer mic → haptic → recording bar (live waveform + duration + slide-to-cancel). Mic hold steals the composer pan. Slide left past `layout.voiceCancelTranslation` (80) discards; release under the threshold sends; slide up `voiceLockTranslation` (80) locks. Locked buttons: Pause · Preview · Send · Discard, plus VoiceOver actions. Voice bubble: waveform · duration · play/pause · 1×/1.5×/2×. `onSendVoice(URL, duration, waveform, quote?)`. Recording bar is the composer family; waveform is accent at ~60%; cancel hint goes secondary → destructive red; lock capsule uses `materials.composer` and ultraLight 22 icons. Everyday copy only (no competitor names). Attach sheet and a live recording disable the dismiss pan.
