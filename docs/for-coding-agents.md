...
- Fullscreen pager (Ship 5): tap image/video opens a `UIPageViewController` over the thread (pinch-zoom, interactive dismiss). Chrome is Close · Save · Forward — SF ultraLight, header-thin material, host `onSaveMedia` / `onForward`. No second accent. No “All media” grid. Long-press stays the targeted bubble preview (`previewProvider` nil, no preview-commit action).
