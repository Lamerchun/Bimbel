...
4. `collectionView.keyboardDismissMode = .interactive` for list-started dismiss. Chrome (Plus / pill background / camera) lives in a bouncing `composer.dismiss.scroll` with `.interactive` so a downward pan **actually** drives system dismiss — do not inject touches into the list pan. The Message field caret/selection and hold-mic do not start that scroll. Keyboard + composer move as one on `keyboardLayoutGuide`. Drag far enough and release to complete; a short drag snaps back.
...
7. Voice (SLICE-3-TOKENS): recording bar = composer pill (40, fill, no stroke). Waveform accent @ 0.6 / played @ 1.0. Slide left 72pt discards (hint caption1 → systemRed). Slide up 56pt locks. Plus/Camera dim 0.35; mic stays accent. Locked: Pause · Preview · Send · Discard on `materials.composer` radius 22. Bubble: play circle 32, waveform 24, speed chip 1×/1.5×/2×. Host `onSendVoice(URL, duration, waveform, quote?)`. Composer-Drag is pan-through only — no handle.
